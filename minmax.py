...
def generate_features(unionlist,word,freqeh,le,lh):
    ...
    while j<=(wordlen-count):
        ...
        while t<lh:
            if unionlist[t]==individual:
                feature_vector[0][t]=freqeh[t]
            t+=1
        t=0    
        while t<le:
            if unionlist[t+lh]==individual:
                feature_vector[0][t+lh]=freqeh[t+lh]
            t+=1     
        j+=1
...
while i<no_eng:
    ...
    freq_eng[((abs(ord(bigrams_eng[i][0])-97))*26+abs(ord(bigrams_eng[i][1])-97))%(26*26)]+=1
    i=i+1
...
flag=0    
while i<len(removed_hindi):
    while j<len(removed_english):
        if removed_hindi[i]==removed_english[j]:
            
            flag=1
            if removed_hindi_freq[i]>removed_english_freq[j]:
                freqeh.append(1)
            else:
                freqeh.append(0)
        j=j+1 
...
flag=0    
while i<len(removed_english):
    while j<len(removed_hindi):
        if removed_hindi[j]==removed_english[i]:
           
            flag=1
            if removed_hindi_freq[j]<removed_english_freq[i]:
                freqeh.append(1)
    ...
    i=i+1
    j=0
print len(removed_english)+len(removed_hindi)
print len(freqeh)    
#numpy.zeros(shape=(1,len(unionlist)))
p=generate_features(unionlist,'kutta',freqeh,len(removed_english),len(removed_hindi))
#dataset will have words in vertical and feature vector for each of them
...
m=0
while i<length:
    dataset[i]=generate_features(unionlist,english_words[i],freqeh,len(removed_english),len(removed_hindi))
    if i<int((4*length)/5):
        x_train[k]=dataset[i]
        y_train[k]=1
        k=k+1
...
while j<length:
    dataset[i]=generate_features(unionlist,english[j],freqeh,len(removed_english),len(removed_hindi))
    if j<int((4*length)/5):
        x_train[k]=dataset[i]
        y_train[k]=0
        k=k+1
...
print "logistic regression acc - " , accuracy_score(y_test,y_pred)
...
print "SVM acc - " ,accuracy_score(y_test,clf.predict(x_test))
...
print "Decision Tree - " ,accuracy_score(y_test,clf.predict(x_test))
"""